<template>
  <button v-if="to == ''" type="submit" class="relative btn" :class="btnClass" :disabled="loading || disabled">
    <BaseSpinner :loading="loading" />
    <slot />
  </button>
  <NuxtLink v-else class="btn" :class="btnClass" :to="to">
    <slot />
  </NuxtLink>
</template>
...
<style lang="scss">
.btn {
    @apply bg-green-700 inline-block w-full px-2 py-2 rounded cursor-pointer text-xl font-bold text-center text-white mb-1 transition-colors outline-none;
